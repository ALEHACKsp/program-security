...
__forceinline std::uintptr_t hide_thread()
{
	...
	if( status != 0x0 )
	{
		std::printf("[  security   ] NtSetInformationThread | 0x0\n\n");
		std::printf("[  security   ] Starting execute syscall\n");
	...
	std::printf("[    security    ] NtSetInformationThread | 0x1\n");
...
__forceinline std::uintptr_t check_debug_flags()
{
	...
	{
		std::printf("[    security   ] NtQueryInformationProcess Syscall | 0x0\n");
	}
